...
The the STAC Items shall follow the following profile.
...
| assets[].href | string | **REQUIRED.** Each asset of the STAC Item containing an URI as \`href\` is ingored and will be posted into the Catalogue as-is. Each asset of the STAC Item containing a local path as \`href\` is transferred from the stagein area to the main storage area. It shall thus have: An \`href\` pointing to the relative path of the asset binary file into the S3 stagein bucket associated to the Collection. An \`href\` filename containing only the [a-zA-Z0-9._-] characters (the rest of the filename path is ignored). The path cannot point to a directory, only a file. The filename shall have maximum 100 characters. An \`href\` filename unique among all the STAC Item assets (two assets in the same STAC item cannot have the same filename, even if they have different relative paths)  |
| assets[].type | string | **REQUIRED.** [Media type](#media-types) of the asset. It is highly reommended to use Cloud-native formats such as COG or Zarr. See the [common media types](../best-practices.md#co) in the best practice doc for commonly used asset types. |      |
| assets[].roles | \[string] | **REQUIRED.** The [semantic roles](#roles) of the asset, similar to the use of `rel` in links. |
  ...
  if 'assets' in i:
    ...
    for asset_key in assets_dict:
      ...
      if '://' not in asset['href']:
        ...
        #Check the file size matches the one in the metadata
        staging_asset_filesize=os.path.getsize(staging_asset_path)
        if staging_asset_filesize != asset['file:size']: return {"id":i['id'],"failure_reason":f"Asset {asset_key} is invalid. File size does not match the one of the file in the S3 stagein path"} 
    ...
    #Check at least one asset with role 'data' is provided
    ...
    if data_is_present == False:
      return {"id":i['id'],"failure_reason":f"At least one asset with role 'data' is mandatory."}
  else:
    #Error, one asset with data is mandatory
    return {"id":i['id'],"failure_reason":f"No asset provided. At least one asset with role 'data' is mandatory."}
